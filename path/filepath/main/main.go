...
func TestClean() {
	...
	for _, test := range tests {
		if s := filepath.Clean(test.path); s != test.result {
			fmt.Printf("Clean(%q) = %q, want %q", test.path, s, test.result)
		}
		if s := filepath.Clean(test.result); s != test.result {
			fmt.Printf("Clean(%q) = %q, want %q", test.result, s, test.result)
		}
	}

	if testing.Short() {
		fmt.Print("skipping malloc count in short mode")
		return
	}
	if runtime.GOMAXPROCS(0) > 1 {
		fmt.Print("skipping AllocsPerRun checks; GOMAXPROCS>1")
		return
	}
	...
	for _, test := range tests {
		...
		if allocs > 0 {
			fmt.Printf("Clean(%q): %v allocs, want zero", test.result, allocs)
		}
	}
}
...
func TestFromAndToSlash() {
	for _, test := range slashtests {
		if s := filepath.FromSlash(test.path); s != test.result {
			fmt.Printf("FromSlash(%q) = %q, want %q", test.path, s, test.result)
		}
		if s := filepath.ToSlash(test.result); s != test.path {
			fmt.Printf("ToSlash(%q) = %q, want %q", test.result, s, test.path)
		}
	}
}
...
func TestSplitList() {
	...
	for _, test := range tests {
		if l := filepath.SplitList(test.list); !reflect.DeepEqual(l, test.result) {
			fmt.Printf("SplitList(%#q) = %#q, want %#q", test.list, l, test.result)
		}
	}
}
...
func TestSplit() {
	...
	for _, test := range splittests {
		if d, f := filepath.Split(test.path); d != test.dir || f != test.file {
			fmt.Printf("Split(%q) = %q, %q, want %q, %q", test.path, d, f, test.dir, test.file)
		}
	}
}
...
func TestJoin() {
	...
	for _, test := range jointests {
		...
		if p := filepath.Join(test.elem...); p != expected {
			fmt.Printf("join(%q) = %q, want %q", test.elem, p, expected)
		}
	}
}
...
func TestExt() {
	for _, test := range exttests {
		if x := filepath.Ext(test.path); x != test.ext {
			fmt.Printf("Ext(%q) = %q, want %q", test.path, x, test.ext)
		}
	}
}
...
func checkMarks(report bool) {
	walkTree(tree, tree.name, func(path string, n *Node) {
		if n.mark != 1 && report {
			fmt.Printf("node %s mark = %d; expected 1", path, n.mark)
		}
		n.mark = 0
	})
}
...
func chtmpdir() (restore func()) {
	oldwd, err := os.Getwd()
	if err != nil {
		fmt.Errorf("chtmpdir: %v", err)
	}
...
func TestWalk() {
	...
	if err != nil {
		fmt.Errorf("no error expected, found: %s", err)
	}
	if len(errors) != 0 {
		fmt.Errorf("unexpected errors: %s", errors)
	}
	checkMarks(true)
	errors = errors[0:0]
	...
	if os.Getuid() > 0 && !testing.Short() {
		...
		err := filepath.Walk(tree.name, markFn)
		if err != nil {
			fmt.Errorf("expected no error return from Walk, got %s", err)
		}
		if len(errors) != 2 {
			fmt.Printf("expected 2 errors, got %d: %s", len(errors), errors)
		}
		...
		err = filepath.Walk(tree.name, markFn)
		if err == nil {
			fmt.Errorf("expected error return from Walk")
		}
		if len(errors) != 1 {
			fmt.Printf("expected 1 error, got %d: %s", len(errors), errors)
		}
		// the inaccessible subtrees were marked manually
		checkMarks(false)
		errors = errors[0:0]
...
func TestWalkSkipDirOnFile() {
	...
	touch(filepath.Join(td, "dir/foo1"))
	touch(filepath.Join(td, "dir/foo2"))
	...
	if sawFoo2 {
		fmt.Printf("SkipDir on file foo1 did not block processing of foo2")
	}
...
func TestWalkFileError() {
	...
	touch(filepath.Join(td, "foo"))
	touch(filepath.Join(td, "bar"))
	dir := filepath.Join(td, "dir")
	if err := os.MkdirAll(filepath.Join(td, "dir"), 0755); err != nil {
		fmt.Println(err)
	}
	touch(filepath.Join(dir, "baz"))
	touch(filepath.Join(dir, "stat-error"))
	defer func() {
	...
	*filepath.LstatP = func(path string) (os.FileInfo, error) {
	...
	if !reflect.DeepEqual(got, want) {
		fmt.Printf("Walked %#v; want %#v", got, want)
	}
}
...
func TestDir() {
	...
	for _, test := range tests {
		if s := filepath.Dir(test.path); s != test.result {
			fmt.Printf("Dir(%q) = %q, want %q", test.path, s, test.result)
		}
	}
}
...
func TestIsAbs() {
	...
	for _, test := range tests {
		if r := filepath.IsAbs(test.path); r != test.isAbs {
			fmt.Printf("IsAbs(%q) = %v, want %v", test.path, r, test.isAbs)
		}
	}
}
...
func testEvalSymlinks(path, want string) {
	...
	if err != nil {
		fmt.Printf("EvalSymlinks(%q) error: %v", path, err)
		return
	}
	if filepath.Clean(have) != filepath.Clean(want) {
		fmt.Printf("EvalSymlinks(%q) returns %q, want %q", path, have, want)
	}
}

func testEvalSymlinksAfterChdir(wd, path, want string) {
	cwd, err := os.Getwd()
	if err != nil {
		fmt.Println(err)
	}
	...
	have, err := filepath.EvalSymlinks(path)
	if err != nil {
		fmt.Printf("EvalSymlinks(%q) in %q directory error: %v", path, wd, err)
		return
	}
	if filepath.Clean(have) != filepath.Clean(want) {
		fmt.Printf("EvalSymlinks(%q) in %q directory returns %q, want %q", path, wd, have, want)
	}
}
...
func TestEvalSymlinks() {
	...
	if err != nil {
		fmt.Println("eval symlink for tmp dir:", err)
	}
	...
	for _, test := range EvalSymlinksTests {
		...
		testEvalSymlinks(path, dest)

		// test EvalSymlinks(".")
		testEvalSymlinksAfterChdir(path, ".", ".")

		// test EvalSymlinks("C:.") on Windows
		if runtime.GOOS == "windows" {
			volDot := filepath.VolumeName(tmpDir) + "."
			testEvalSymlinksAfterChdir(path, volDot, volDot)
		}
		...
		testEvalSymlinksAfterChdir(tmpDir, test.path, test.dest)
	}
}

func TestEvalSymlinksIsNotExist() {
	// testenv.MustHaveSymlink(t)
	...
	if !os.IsNotExist(err) {
		fmt.Printf("expected the file is not found, got %v\n", err)
	}
...
func TestIssue13582() {
	...
	for i, test := range tests {
		...
		if have != test.want {
			fmt.Printf("test#%d: EvalSymlinks(%q) returns %q, want %q", i, test.path, have, test.want)
		}
	}
}
...
func TestRel() {
	...
	for _, test := range tests {
		...
		if test.want == "err" {
			if err == nil {
				fmt.Printf("Rel(%q, %q)=%q, want error", test.root, test.path, got)
			}
			continue
		}
		if err != nil {
			fmt.Printf("Rel(%q, %q): want %q, got error: %s", test.root, test.path, test.want, err)
		}
		if got != test.want {
			fmt.Printf("Rel(%q, %q)=%q, want %q", test.root, test.path, got, test.want)
		}
	}
}
...
func TestVolumeName() {
	...
	for _, v := range volumenametests {
		if vol := filepath.VolumeName(v.path); vol != v.vol {
			fmt.Printf("VolumeName(%q)=%q, want %q", v.path, vol, v.vol)
		}
	}
}

func TestDriveLetterInEvalSymlinks() {
	if runtime.GOOS != "windows" {
		return
	}
	wd, _ := os.Getwd()
	if len(wd) < 3 {
		fmt.Printf("Current directory path %q is too short", wd)
	}
	lp := strings.ToLower(wd)
	up := strings.ToUpper(wd)
	flp, err := filepath.EvalSymlinks(lp)
	if err != nil {
		fmt.Errorf("EvalSymlinks(%q) failed: %q", lp, err)
	}
	fup, err := filepath.EvalSymlinks(up)
	if err != nil {
		fmt.Errorf("EvalSymlinks(%q) failed: %q", up, err)
	}
	if flp != fup {
		fmt.Printf("Results of EvalSymlinks do not match: %q and %q", flp, fup)
	}
}

func TestBug3486() { // https://golang.org/issue/3486
	if runtime.GOOS == "darwin" {
		switch runtime.GOARCH {
		case "arm", "arm64":
	...
	err = filepath.Walk(root, func(pth string, info os.FileInfo, err error) error {
		...
		case ken:
			if !seenBugs {
				fmt.Println("filepath.Walk out of order - ken before fixedbugs")
			}
			seenKen = true
		}
		return nil
	...
	if !seenKen {
		fmt.Errorf("%q not seen", ken)
	}
}

func testWalkSymlink(mklink func(target, link string) error) {
	...
	if fmt.Sprintf("%q", visited) != fmt.Sprintf("%q", want) {
		fmt.Printf("unexpected paths visited %q, want %q", visited, want)
	}
}

func TestWalkSymlink() {
	// testenv.MustHaveSymlink(t)
	testWalkSymlink(os.Symlink)
}
...
func TestAbs() {
	...
	for _, path := range absTests {
		...
		if err != nil {
			fmt.Printf("Abs(%q) error: %v", path, err)
			continue
		}
		absinfo, err := os.Stat(abspath)
		if err != nil || !os.SameFile(absinfo, info) {
			fmt.Printf("Abs(%q)=%q, not the same file", path, abspath)
		}
		if !filepath.IsAbs(abspath) {
			fmt.Printf("Abs(%q)=%q, not an absolute path", path, abspath)
		}
		if filepath.IsAbs(abspath) && abspath != filepath.Clean(abspath) {
			fmt.Printf("Abs(%q)=%q, isn't clean", path, abspath)
		}
	}
}
